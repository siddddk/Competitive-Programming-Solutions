...
void solve(void) 
{
    ...
    vector<vector<Mint>> dp(primes.size() + 1, vector<Mint> (n + 1, 0));
    dp[primes.size()][0] = 1;
    for(int i = primes.size() - 1; i >= 0; i--) {
      for(int j = 0; j <= n; j++) {
        dp[i][j] = inv_fact[primes[i]] * dp[i + 1][j];
        if(j > 0) dp[i][j] += inv_fact[primes[i] - 1] * dp[i + 1][j - 1];
      }
    }
    cout << dp[0][n] * ans << "\n";
}
